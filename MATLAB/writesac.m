...
% writesac( u,t0, dt, lat, lon,evlat,evlon, fname, format )
% Input: 
%   u        - The data component on SAC file
%   dt       - Uniform time step for u
%   lat, lon - Latitude and longitude position of the SAC station.
%   evlat, evlon - Latitude and longitude position of source.
%   t0       - start time
%   fname    - Name of SAC file
%   format   - Little endian ('l') or big endian ('b') byte order for binary data. Default is 'l'.

function [] = writesac(u,t0,dt,lat,lon,evlat,evlon,fname,format)
if nargin < 9
    format = 'l';
end
...
fdum = -12345.0;
idum = -12345;
% too much char cause 0 in data
% 3 char 96 ge 0
% 2 char 48 ge 0
% we should keep only one char to balance the size
cdum = 'k';
iftype=1;
fid = fopen(fname,'wb',format,'US-ASCII');
if fid < 0
